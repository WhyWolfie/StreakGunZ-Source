...
//	m_pOcclusion=NULL;
	...
	m_bWireframe=false; // Custom: Wireframe
...
void RBspObject::SetDiffuseMap(int nMaterial)
{
	...
	if (GetWhiteMap() == 1)
	{
		//much darker gray (very easy on the eyes)
	...
	else
	{
		...
		}else
		{
			...
			else
			{
				...
				pd3dDevice->SetSamplerState(0, D3DSAMP_MIPFILTER, bTrilinear ? D3DTEXF_LINEAR : D3DTEXF_NONE);
				pd3dDevice->SetSamplerState(0, D3DSAMP_MIPMAPLODBIAS, -1);
				...
				pd3dDevice->SetSamplerState(1, D3DSAMP_MIPFILTER, bTrilinear ? D3DTEXF_LINEAR : D3DTEXF_NONE);
				pd3dDevice->SetSamplerState(1, D3DSAMP_MIPMAPLODBIAS, -1);
			}
...
//		if(hr!=D3D_OK) 
//			mlog("lightmap texture failed to create: %s \n",DXGetErrorString9(hr));
