...
void AniFrameInfo::Frame(RAniMode amode,RVisualMesh* pVMesh)
{
	...
	DWORD cur = timeGetTime();//¿ÅE™ ≈∏¿Ã∏” ª˝±Ê∂ß±˚›ÅE.
	...
	if(m_bChangeAnimation) {
		...
		if(m_pAniIdEventSet !=NULL)
			m_pAniNameEventSet = m_pAniIdEventSet->GetAniNameEventSet( m_pAniSet->GetName());	//¿œ¥‹ «ÿ¥ÅEæ÷¥œ∏ﬁ¿Ãº« º¬¿ª √£∞ÅE..
		if(m_pAniNameEventSet != NULL)
		{
			m_vecCheckAniEvent.clear();		//±‚¡∏¿« «√∑°±◊¥¬ ≥Ø∑¡πˆ∏Æ∞ÅE
		...
		m_nFrame = m_nAddFrame;	// ∏º«ªÛ≈¬¥¬ ∞∞∞ÅEπ´±‚∏∏ πŸ≤ÅEÊøÅE∂ßπÆø°

		if(m_pAniSet->IsHaveSoundFile()) {
			RAniSoundInfo* pSInfo = &m_SoundInfo;

			if(strcmp(m_pAniSet->GetName(),"jumpD") != 0) // ¿”Ω√ ¡°«¡ ¥Ÿø˚‹∫ ªÁø˚—ÅE∆ƒ¿œ ¡§∫∏∏∏ ∞°¡ˆ∞ÅE¬¯¡ÅEΩ√¡°ø° play
				pSInfo->isPlay = true;
	...
	if( m_isPlayDone ) {// 1π¯¿∫ play «—»ƒ..
		...
		if( m_pAniSet->GetAnimationLoopType() == RAniLoopType_Loop)
		{
			...
			if(m_pAniNameEventSet != NULL)
			{
				m_vecCheckAniEvent.clear();		//¿œ¥‹ ±‚¡∏ø° Ω◊ø©¿÷¥ÅE«√∑°±◊µÈ¿ª ≥Ø∑¡πˆ∏Æ∞ÅE
				m_pAniNameEventSet->m_AniNameEventSetIter = m_pAniNameEventSet->m_AniNameEventSet.begin();
				while( m_pAniNameEventSet->m_AniNameEventSetIter != m_pAniNameEventSet->m_AniNameEventSet.end())
				{
					m_vecCheckAniEvent.push_back(false);
			...
			} else 	{	//	¥Ÿ¿ΩπÅEµ˚”œ¿Ã æ¯¥Ÿ∏ÅE.
				//	∏º« ¿Ã∏ß¿∏∑Œ µ»∞ÕµÅEªÛ≈¬µÈ¿Ã ¡§«ÿ¡ˆ∏ÅEID ∑Œ πŸ≤Ÿ±ÅE..
...
//	m_fSpeed = 2.1f;
	...
	if ( m_nFrame >=  ef) {
		...
		m_isPlayDone		= true;	// loop ãö∏∂¥Ÿ..
		m_isOncePlayDone	= true; // ø°¥œ∞° πŸ≤∂ß±˚›ÅEæ»∫Ø«‘. (1π¯¿∫ «√∑°¿Ã µ«æ˙¥Ÿ¥¬∞… ∫∏¿ÅE
		m_save_time			= cur;
	}

	m_1frame_time = cur;
	//æ÷¥œ∏ﬁ¿Ãº« ¿Ã∫•∆Æ »Æ¿Œ¿ª ¿ß«— ∫Œ∫–..
	if(m_pAniNameEventSet != NULL)	//æ÷¥œ∏ﬁ¿Ãº« ¿Ã∫•∆Æ ¡§∫∏∞° ¿÷¿∏∏ÅE
	{
		m_pAniNameEventSet->m_AniNameEventSetIter = m_pAniNameEventSet->m_AniNameEventSet.begin();
...
// ¿€µø æ»«‘ ¿ÅE™≈∏¿Ã∏”∞° æ¯¥Ÿ∏ÅE∆Ω¿ª ∫∏∞ÅEÿ ¡‡æﬂ«‘...∫∞∑Œ ªÁøÅE“ « ø‰∞° æ¯¿ª∞Õ ∞∞¿Ω..
void RVisualMesh::Pause(RAniMode amode) {

	AniFrameInfo* pInfo = GetFrameInfo(amode);
	if( pInfo == NULL ) return;

	m_nAnimationState = APState_Pause;//m_nFrame ¿∫ ¿Ø¡ÅE.
...
void RVisualMesh::Render(bool low,bool render_buffer) {
	...
	if(m_pMesh) {
		...
		/*	
		if(m_isScale) { // Ω∫ƒ…¿œ¿Ã ªÁøÅE» ∞ÊøÅE∏...
...
//		if( m_pMesh->m_bEffectSort ) {
			...
			if(m_fVis < 0.001f) {//¡◊æÅE¿÷¥¬ ∞ÊøÅEÅE.
				m_bIsRender = false;
				return;
			}

			if(m_bCheckViewFrustum) {//tool ¿Ã∂Û∏ÅEfalse
...
//		}
		...
		// ∏ÛΩ∫≈ÕøÕ ƒ≥∏Ø≈Õ √ﬂ∞°«œ∞ÅE ( ƒ≥∏Ø≈Õµµ ¿⁄Ω≈¿« ∞Õ¿∫ √º≈©æ»«œ∞ÅE) 
		//  πÃ∏Æ∞ËªÅEÿ¡Ÿ∞ÕµÅE.
		//	∞≈∏Æ∞° ∞°±˚€ˆº≠ ∫∏¿œ∞≈∂Û∞ÅE∆«¥‹µ«¥¬∞Õ¿∫ ªÁøÅE»«‘,, 
		//  ƒ≥∏Ø≈Õ≥™ ∏ÛΩ∫≈Õ¥¬ ±‚∫ª¿˚¿∏∑Œ picking «—πÅE«ÿ∫¡º≠ ∫Æ¿Ã æ»¡˝»˜∏ÅE.±◊∑¡¡÷±ÅE.( π´¡∂∞« ∫∏¿Œ¥Ÿ¥¬ ∂Ê¿Ã¥œ±ÅE)
		// ¿Ã∆Â∆Æ¥¬? ¿Œ≈Õ∆‰¿ÃΩ∫¥¬ π´¡∂∞« off m_bQuery ∑Œ ªÁøÅE“¡ÅEø©∫Œ∏¶ visual mesh ∞° ∞°¡ˆ∞ÅE¿÷±ÅE.
		// 
/*
		if( RIsQuery() ) {
...
*/
		...
		m_bIsRender = true; // ±◊∑¡¡≥¥Ÿ∞ÅE«•Ω√

		// ∏µ® ∫Œ¿ß∫∞∑Œ scale ∏‘¿Ã¥¬∞Õ ∞˙”¡..√º«ÅEª ∞·¡§«“ ºÅE¿÷∞‘ µ»¥Ÿ..
		...
		if(low) {
			...
			if(m_pLowPolyMesh) {
				...
				m_pLowPolyMesh->Render(&m_WorldMat,true); // ∆ƒ√˜±≥»Øæ¯¿Ã ¿⁄Ω≈∏∏ ±◊∏Æ±ÅE.
...
/*
			...
			if(m_pMesh->GetPhysiqueMesh()) { 
				...
				// RMesh::m_max, m_min ¡¶¥ÅEŒ ∞ËªÅE«æ˚‹÷¡ÅEæ ¥Ÿ.
				/*
...
bool RVisualMesh::UpdateSpWeaponFire()
{
	...
	m_bGrenadeRenderOnoff = true;// ±‚∫ª¿˚¿∏∑Œ «◊ªÅE∫∏¿Œ¥Ÿ...

	if( m_bDrawGrenade ) {// ≥≤¿∫ æÁ¿Ã æ¯¥Ÿ∏ÅE¡„∞ÅE¿÷¡ˆµµ æ ¥¬¥Ÿ~

		if(pAniUp->m_pAniSet) {

			if( strcmp(pAniUp->m_pAniSet->GetName(),"attackS")==0 ) {

				if( m_bGrenadeFire ) { // πﬂªÅEªÛ≈¬∂Û∏ÅE
...
//	if(timeGetTime()%3==0)
	...
	// ±‚∫ª¿˚¿∏∑Œ¡ˆ±›π´±‚¿« bbox ∑Œ ªÛ¿ß∑Œ «‚«œ¥¬ ∫“±Ê¿ª ∏∏µÁ¥Ÿ..
...
void RVisualMesh::RenderWeapon()
{
	if( m_WeaponVisualMesh[ m_SelectWeaponMotionType ] ) {
		...
		m_LightMgr.Clone(pVWMesh);//∂Û¿Ã∆Æ º≥¡§ ¡§∫∏∏¶ ∫πªÅE..
		
		bool b2hCheck = false;	// æÁº’¿Œ¡ÅE√º≈©
		bool bRender = true;
		...
		if( bRender && vis) {
			pVWMesh->SetVisibility(min(m_fVis,vis)); // ¿⁄±ÅEƒ≥∏Ø≈ÕøÕ ¿⁄Ω≈¿« visibility ¡ﬂ ¥ÅE¿€¿∫∞Õ..
			pVWMesh->Render();
			m_bIsRenderWeapon = true;
		}
		else { // ¿ßƒ°¥¬ æµ¡ˆµµ ∏∏£¥œ ∞ËªÅEÿ¡÷¿⁄..ex > ºˆ∑˘≈∫¿ª ¥¯¡¯∞ÊøÅE.
			pVWMesh->CalcBox();//√÷√ ∏∏∞ËªÅE
			m_bIsRenderWeapon = false;
		}
...
void RVisualMesh::DrawTracks(bool draw,RVisualMesh* pVWMesh,int mode,rmatrix& m) 
{
	...
	if( draw && m_bDrawTracksMotion[mode] ) {
		...
		// ƒÆ ¡æ∑˘ø° µ˚∂Ûº≠ ±Ê¿Ã∞° ∆≤∏Æµµ∑œ ¿˚øÅEÿ∫∏±ÅE.
		// ¿Ãµµ∑ÅE, ƒ´≈∏≥™ , ¥ÅEÀ.. ¿Ãµµ∑˘¿« »÷æ˚›ÅEƒÆ¿Ã πÆ¡¶..
	...
	if(m_pMesh) {
		...
		if(m_pMesh->GetPhysiqueMesh()) {
			...
			if(m_bIsRenderFirst) {
				...
				//ø£¡¯ƒ⁄µÅE∫∏¥œ scale º∫∫–¿Ã worldmatøÕ µ˚∑Œ ∞ÅEÆµ«∞ÅE¿÷±ÅE∂ßπÆø° scale¿ª ∞∞¿Ã ≥÷æ˚ÿ≠ πŸø˚—˘π⁄Ω∫∏¶ ∞ËªÅEœ¥¬∞‘ ∏¬¥Ÿ.
				//±◊∑Ø≥™ ºˆ≥‚∞£ ±◊∑∏¡ÅEæ ¿∫ ƒ⁄µÂ∑Œ º≠∫ÒΩ∫«—¡ˆ∂ÅE∞˙‡°±ÅEπ´º∑¥Ÿ. « ø‰«œ¥Ÿ∏ÅEºˆµø¿∏∑Œ m_bCalcBoxWithScale=true∏¶ ¡÷æ˚ÿ≠ ∞ËªÅEœ¿⁄
				...
				m_vBMax = m_pMesh->m_vBBMaxNodeMatrix * 1.2f;// 20% ¡§µµ ¥ÅE≈©∞‘
...
void RVisualMesh::AddWeapon(RWeaponMotionType type,RMesh* pMesh,RAnimation* pAni)
{
	if((type < eq_weapon_etc)||(type > eq_weapon_end-1)) {
		mlog("RVisualMesh::AddWeapon ¥œ∞° RVisualMesh ∏¶ ±˙∏‘¥¬±∏≥™~~~\n");
	}
...
// ¿”Ω√ ¿Ã∏ß¿∏∑Œ ø°¥œ∏ﬁ¿Ãº« ≈∏¿Ÿ ¡§«ÿ¡÷±ÅE

enum apm_type {
	apm_attack1 = 1,// ani play mode -> «—π¯«œ∞ÅEidle ∑Œ πŸ≤ÅE
	apm_attack2,	// ∆Øºˆ∞¯∞›µÅE.
	apm_walk,		
	apm_run,		// loop
	apm_die,
...
//	bool bChange = true;
	...
	if(b) { 
		...
		if( pInfo->m_pAniSet ) {
			if( pInfo->m_pAniSet != pAniSet) {//∏µ® ∆˜¿Œ≈Õ∏∏ πŸ≤ÅEºÅEµµ ¿÷¥Ÿ..
				bChange = true;
			}
		}
		else {
	...
	if(bChange) {
		...
		// ∏º«¿∫ πŸ≤˚⁄˙¿∏≥™ ªÛ≈¬∞° ∞∞¥Ÿ∏ÅE(π´±‚∏∏ πŸ≤€∞ÊøÅE)¿Ã¿ÅE«¡∑π¿” ¿Ø¡ÅE
...
void RVisualMesh::UpdateMotionTable()
{
	...
	if( m_nAniNodeTableCnt != meshnode_cnt) {//∏µ®¿Ã πŸ≤¿œ¿∫ æ¯¡ˆ∏∏..
		if(m_pAniNodeTable) {
			delete [] m_pAniNodeTable;
			m_pAniNodeTable = NULL;
		}
...
// ƒ≥∏Ø≈Õ ªÛ¿ßø°º≠ ¿⁄Ω≈¿« ªÛ≈¬øÕ µÈ∞ÅE¿÷¥¬ π´±‚ø° µ˚∂Ûº≠ ø°¥œ∏ﬁ¿Ãº« ø¨∞·«ÿ¡÷±ÅE
...
//			m_pAniSet[ani_mode_upper] = NULL;
	...
	if(ani_name[0]==0) {//¿Ã∏ß¿Ã æ¯¿ª ∞ÊøÅE
		if(animode==ani_mode_upper){
			pAniUp->m_pAniSet = NULL;
//			m_pAniSet[ani_mode_upper] = NULL;
		}
	...
	if(m_SelectWeaponMotionType!=eq_weapon_etc)//±‚∫ª∞™¿Ã∏ÅE±◊≥… -1
		wtype = GetSetectedWeaponMotionID();

	if(animode == ani_mode_upper) {
		if( pAniLow->m_pAniSet ) {
			// load,reload ¥¬ ªÛ∞ÅE»«‘..
			// ∞¯∞›¿« ∞ÊøÅE°¥¬ idle ªÛ≈¬≥™ ƒÆ¿Ã≥™ ºˆ∑˘≈∫¿Œ ∞ÊøÅE∏ ºØæ˚›ÿ¥Ÿ..
			if(strcmp(ani_name,"attackS")==0 ) {

				if(strcmp( pAniLow->m_pAniSet->GetName() ,"idle") != 0) {//idle ¥¬ π´¡∂∞«..±◊ø‹ø°¥¬..
...
//						(wtype != eq_wd_pistol) &&
						(wtype != eq_wd_shotgun) /*&&
						//¿‹≈∫ 0¿œ∂ß ¥Ÿ∏• π´±‚∑Œ ¿⁄µø ¿ÅEØ«œ¥¬ ±‚¥…¿Ã ªÛ√º æ÷¥œ øœ∑ÅEΩ√¡°ø° ¿€µø«œ¥¬µ•
						//idleø‹ø° æ∆¿Ã≈€ º¶ æ÷¥œ∏¶ ª˝∑´«ÿπˆ∏Æ∏ÅE¿⁄µø ¿ÅEØ ±‚¥…¿Ã ¿€µø«œ¡ÅEæ ∞‘ µ«æ˚ÿ≠ æ∆¿Ã≈€µµ æ÷¥œ ºØ±‚∞° « ø‰«œ¥Ÿ
						(wtype != eq_wd_item)
						*/ 
						// °ÅE¿Ø¿˙µÈ¿∫ ºˆ∏Æ≈∂ ¿«∑·≈∂¿ª ∫Æ¿ª ∫∏∞ÅE¥ﬁ∏Æ∏Èº≠ π¯∞•æ∆ ¥¯¡Æº≠ »ƒ¥Ÿ¥⁄ ∏‘¥¬ πÊπ˝¿ª æ≤∞ÅE¿÷æ˙¥Ÿ
						// ¥ﬁ∏Æ∏Èº≠ ¥¯¡ˆ∏ÅE¥¯¡ˆ¥¬ æ÷¥œ∞° ª˝∑´µ«æ˚ÿ≠ ª°∏Æ ¥¯¡ÅEºÅE¿÷±ÅE∂ßπÆ
						// ¿ßø°º≠ æ÷¥œ ª˝∑´¿ª «œ¡ˆæ µµ∑œ «ﬂ¥ı¥œ ºˆ∏Æ≈∂ ¿«∑·≈∂ ¥¯¡ˆ¥¬ æ÷¥œ∏ﬁ¿Ãº«¿Ã ¥ŸΩ√ ª˝∞‹≥™º≠ ¿Ø¿˙µÈ¿Ã
						// µÙ∑π¿Ã∞° ¥¿≤∏¡¯¥Ÿ∞ÅE«◊¿«∏¶ «—¥Ÿ. (∆Ø»ÅEƒ≥Ω¨ ¿«∑·≈∂¿ª ªÅE¿Ø¿˙µÅE
						// ¥Ÿæ¥π´±ÅE¿⁄µø¿ÅEØ æ»µ«¥¬ πˆ±◊∏¶ ºˆ¡§«œ±‚∫∏¥Ÿ¥¬ ±‚¡∏ «√∑π¿Ã∞®¿ª ¿Ø¡ˆ«œ¥¬ ∆˙‹Ã ≥¥¥Ÿ∞ÅE∆«¥‹«œø© ¥ŸΩ√ ¡÷ºÆ√≥∏Æ«‘
						)
					{
						pAniUp->m_pAniSet = NULL;
						return true;
...
bool RVisualMesh::GetWeaponDummyMatrix(WeaponDummyType type,rmatrix* mat,bool bLeft)
{
	if(type > weapon_dummy_etc && type < weapon_dummy_end)
	{
		...
		if(m_bIsNpc)//npc ¿«∞ÊøÅE¬ scale √≥∏ÆøÕ worldmatrix √≥∏Æ∞° ¿⁄µø¿∏∑Œ æ» µ«æ˚‹÷¥Ÿ..
		{
			if(m_isScale)
				m = m_ScaleMat * m;
			m = m * m_WorldMat;
...
// π´±ÅE∏µ®ø° ¥ﬁ∏∞ ¥ıπÃ¿œ ∞ÊøÅE.
void RVisualMesh::UpdateWeaponDummyMatrix(RMeshNode* pMeshNode)
{
	if(!pMeshNode) return;

	if( pMeshNode->m_WeaponDummyType != weapon_dummy_etc) { // π´±‚ø° ¥ﬁ∏∞ ¥ıπÃ
...
bool RVisualMesh::CreateCloth(RMeshNode* pMeshNode,float fAccel,int Numiter)
{
	...
	pMeshNode->m_bClothMeshNodeSkip = false;//±◊∏∞¥Ÿ..
...
bool RVisualMesh::ChangeChestCloth(float fAccel,int Numiter  )
{
	...
	// mesh node∏¶ ∞Àªˆ«œ∏Èº≠ ƒ√∑Ø∞™¿ª ∞°¡ÅE≥ÅEÂ∏¶ ∞…∑Ø≥Ω¥Ÿ

	if( pMeshNode == NULL ) // ∆ƒ√˜∞° æ∆¥— ±‚∫ª ∏µ®ø°º≠ √£æ∆∫ª¥Ÿ..
	{
	...
	// ∞°Ωø ¿Â∫ÅE∏µ®¿Ã πˆ≈ÿΩ∫ ƒ√∑Ø ∞™¿ª ∞°¡ˆ∞ÅE¿÷¥Ÿ∏ÅE..
...
{
	// Custom: Disabled antihack for this stupid function
	/*
	// AniFramInfo
	// m_pVMesh∞° ∞°¡ÅEAniFramInfo¥¬ æ÷¥œ∏ﬁ¿Ãº« øœ∑ÅEbool¿Ã≥™ «ˆ¿ÅE«¡∑π¿”π¯»£ µ˚‹ª ∞Æ∞ÅE¿÷¥Ÿ. ¿Ã∞Õ¿ª ∞˙›§Ω√ƒ— æ÷¥œ ¡¯«‡ªÛ≈¬ø° ¿«¡∏«œ¥¬
	// µÙ∑π¿Ã∏¶ π´»ø»≠«œ¥¬ «ÿ≈∑¿Ã Ω√µµµ  (ƒÆ ±‚∏¿∏±‚∞° ª°∏Æ µ«∞≈≥™, π´±ÅEπﬂªÅE∏Æ∑ŒµÅEæ÷¥œ∏ﬁ¿Ãº«¿ª ª°∏Æ ≥°≥ªπˆ∏Æ∞≈≥™)
	{
	...
	// m_SelectWeaponMotionType ∏¶ ¥‹∞À¿∏∑Œ ∞˙›§«œ∏ÅE∏”Ω≈∞«¿ª µÈ∞ÅE¥˝∫˙‘µ¿Ã æ∆¥— ¥ÅE¨∏¶ «œ∏ÅE∏”Ω≈∞« ∞¯º”¿∏∑Œ ƒÆ¡˙¿Ã ∞°¥…«œ¥Ÿ
	{
		if (m_SelectWeaponMotionType !=
			*m_pSelectWeaponMotionType_AntiHack)
			__asm int 3;	// ≈¨∂ÅE¡◊ø©πˆ∏∞¥Ÿ
...
rmatrix* RVisualMesh::GetMoveMatrix(rmatrix *result)
{
	D3DXMATRIX mat; // [sp+2Ch] [bp-40h]@1
	rvector up = rvector(0, 4.575657221408423936f,1);
	rvector dir = rvector(0, 3.212836864f, 1);
	rvector pos = rvector(m_vPos.x, m_vPos.y + 4, m_vPos.z);
	MakeWorldMatrix(&mat, pos, dir, up);

	memcpy(result, mat, sizeof(D3DXMATRIX));
