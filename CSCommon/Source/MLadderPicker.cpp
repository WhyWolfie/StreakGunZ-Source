...
bool MLadderPicker::PickMatch(int* pGroupA, int* pGroupB, int GameType)
{
	...
	while (true)
	{
		...
		if ((/*pTicketA->GetTriforce() == false && */Evaluate(pTicketA, &itorMatchedTicket))/* || (pTicketA->GetTriforce() == true && Evaluate(pTicketA, &itorMatchedTicket) && nGroupB == 0)*/)
		{
...
{
	// Custom: return true at all times so matchmaking is a lot faster. No need for balancement.
	return true;
