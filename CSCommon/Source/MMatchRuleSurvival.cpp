...
m_nCombatStartTime(0), m_nQuestCompleteTime(0), m_nPlayerCount( 0 )
{
	...
	// Custom: Survival doesn't use scaling, and is planned to be removed with the quest revamp, but let's set default values cause why not.
...
void MMatchRuleSurvival::RouteSectorBonus(const MUID& uidPlayer, unsigned long int nEXPValue, unsigned long int nBP, unsigned long int nPct)
{
	...
	pNewCmd->AddParameter(new MCmdParamUInt(nPct));
