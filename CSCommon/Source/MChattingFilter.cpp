...
//  ret  : filtered string
//*************************************************************************/
//string& MChattingFilter::GetLastFilteredStr()
//{
//	return m_strLastFilterdWord;
//}
//
//
///************************************************************************
...
bool MChattingFilter::LoadFromFile(MZFileSystem* pfs, const char* szFileName)
{
	...
	for (int i = 0; i < iCount; i++)
	{
		...
		if (!strcmp(szTagName, "ABUSEWORD"))
		{
			int nAttrCount = chrElement.GetAttributeCount();
			MAbuseWord* pAbuseWord = new MAbuseWord();
			for (int j = 0; j < nAttrCount; ++j)
			{
				chrElement.GetAttribute(j, szAttrName, szAttrValue);
				if (!stricmp(szAttrName, "name"))
				{
					pAbuseWord->SetAbuseWord(szAttrValue);
				}
				if (!stricmp(szAttrName, "level"))
				{
					pAbuseWord->SetFilteringLevel(atoi(szAttrValue));
				}
			}
			m_AbuseWordList.push_back(move(pAbuseWord));
		}
	}
	xmlIniData.Destroy();
	return true;
