...
void MMatchShop::ParseSellItem(MXmlElement& element)
{
	...
	if (pItemDesc != NULL)
	{
		// Clasificar si es un item special de clase
		if (pNewItemNode->bIsMedalItem)
		{
			//mlog("ItemID : %u StockRate : %.2f\n", pNewItemNode->nItemID, pNewItemNode->fStockRate);
			//random bool
			//if (bSellable(fRatio, nDescID))
			if (yesOrNo(fRatio))
			{
				//if(randomBool())
				//{
				...
				// Dinnos si fue insertado
				//mlog("ItemID : %u StockRate : %.2f\n", pNewItemNode->nItemID, fRatio);
				//}
			}
			else
			{
		...
		{
			// caso contrario solo insertarlos en el nodo
			m_ItemNodeVector.push_back(pNewItemNode);
			m_ItemNodeMap.insert(map<unsigned int, ShopItemNode*>::value_type(pNewItemNode->nItemID, pNewItemNode));
		}
