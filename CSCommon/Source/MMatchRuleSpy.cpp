...
void MMatchRuleSpy::OnRoundEnd()
{
	...
	int nTeamWinner = 0;
	if (m_nRoundArg == 0 || m_nRoundArg == 1 || m_nRoundArg == 3)
	{
		nTeamWinner = MMT_RED;
	}
	else if (m_nRoundArg == 2 || m_nRoundArg == 4)
	{
		nTeamWinner = MMT_BLUE;
	}
	MCommand* pCmd = MGetMatchServer()->CreateCommand(MC_MATCH_SPY_ROUNDRESULT, MUID(0, 0));
	pCmd->AddParameter(new MCmdParamInt(nTeamWinner));
	MGetMatchServer()->RouteToBattle(GetStage()->GetUID(), pCmd);
	return MMatchRule::OnRoundEnd();
...
// Custom: change OnRoundCountdown to OnRoundCountDown
...
#ifdef _SPYLADDER
		const int LADDER_WINNING_ROUNT_COUNT = 2;
#else
...
void MMatchRuleSpy::OnLeaveBattle(MUID& uidChar)
{
	...
	{
#ifdef _BATTLE_MANAGER
		MMatchObject *pObj = MMatchServer::GetInstance()->GetObjectA(*itSpyFound);


		pObj->GetCharInfo()->m_nHP -= pSpyTable->nHPAP;
		pObj->GetCharInfo()->m_nAP -= pSpyTable->nHPAP;
#endif
...
#ifdef _BATTLE_MANAGER
		MMatchObject *pObj = MMatchServer::GetInstance()->GetObjectA(vtCandUIDs.front());
