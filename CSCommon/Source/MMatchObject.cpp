...
{ 
	...
	m_nLadderTeam = MMT_ALL;
	m_uidLadderStage = MUID(0, 0);
	LastVoteID = -1;
	bMatching = false;
}
...
void MMatchObject::SetLadderTeam(MMatchTeam nTeam)
{ 
	m_nLadderTeam = nTeam; 

	if (IsAdminGrade(this) && CheckPlayerFlags(MTD_PlayerFlags_AdminHide))
		m_nLadderTeam = MMT_SPECTATOR;
}
...
void MMatchObject::OnEnterBattle()
{
	...
	/*MMatchChannel* pChannel = MGetMatchServer()->FindChannel(GetChannelUID()); // Custom: Fix Spawn Position Center
	if (pChannel->GetChannelType() != MCHANNEL_TYPE_PLAYERWARS)*/
	ResetGamePlayInfo();
}
