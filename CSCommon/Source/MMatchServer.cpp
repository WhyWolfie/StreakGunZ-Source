...
#include "Ladder/MPlayerWarsMgr.h"
...
bool MMatchServer::LoadChannelPreset()
{
	...
	for (int i = 0; i < iCount; i++)
	{
		...
		if (!strcmp(szTagName, MTOK_CHANNEL))
		{
			...
			childElement.GetAttribute(&bIsPlayerWars, "IsLadder");
...
#define MINTERVAL_GARBAGE_SESSION_PING	(3 * 60 * 1000)	// 3 min
	...
	//System Char Update
	/*static unsigned int UpdateChar = nGlobalClock;
	if (nGlobalClock - UpdateChar > _UPDATECHAR)
	{
		//LOG(LOG_PROG, "Prueba de tiempo");
		UpdateChar = nGlobalClock;
		//GetDBMgr()->UpdateCharSystem();
	}*/
...
void MMatchServer::RouteToStaff(MCommand* pCommand)
{
	for (MMatchObjectList::iterator i = m_Objects.begin(); i != m_Objects.end(); i++) {
		...
		if (pObj->GetUID() < MUID(0, 3)) continue;	// MUID∑Œ Client¿Œ¡ˆ ∆«∫∞«“ºˆ ¿÷¥¬ ƒ⁄µÂ « ø‰«‘
		MCommand* pSendCmd = pCommand->Clone();
		pSendCmd->m_Receiver = pObj->GetUID();
		Post(pSendCmd);
	}
