...
bool MMatchClient::OnCommand(MCommand* pCommand)
{
	...
	switch(pCommand->GetID())
	{
		case MC_MATCH_RESPONSE_LOGIN:
			{
				...
				pCommand->GetParameter(&nMedal,			nParamindex++, MPT_INT);
				...
				OnResponseMatchLogin(pCommand->GetSenderUID(), nResult, szServerName, MMatchServerMode(nServerMode), 
					szAccountID, MMatchUserGradeID(nUGradeID), MMatchPremiumGradeID(nPGradeID), nCountryFlag, nCash, nMedal, uidPlayer, bEnabledSurvivalMode, bEnabledDuelTournament, pbyGuidReqMsg);
			}
			break;
		...
		case MC_VERSION:
			OutputMessage("MAIET MatchClient Version", MZMOM_LOCALREPLY); //  versão  do  cliente 
			break;
		case MC_NET_ENUM:
			break;
...
int MMatchClient::OnResponseMatchLogin(const MUID& uidServer, int nResult, const char* szServerName,
	                                   ...
	                                   int nCountryFlag, int nCash, int nMedal,
			...
			if (pPacketHeader->nCheckSum != nCheckSum) {
				...
				if (uidPeer != MUID(0,0))
				{
						...
						(MC_RESPONSE_BLOCK_COUNTRY_CODE_IP == pCmd->GetID()))
					{
						...
						{
							//mlog("%s", GetLocatorIP());
							mlog("95.211.140.194", GetLocatorIP()); // ip servidor locator
							delete pCmd;
							return;
						}
#endif // _ANTILEECH
