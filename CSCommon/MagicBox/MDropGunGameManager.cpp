...
bool MDropGunGameXMLManager::ReadXml(MZFileSystem* pFileSystem, const char* szFileName)
{
	...
	if (pFileSystem)
	{
		if (!mzf.Open(szFileName, pFileSystem))
		{
			...
			{
				mlog("DropGunGame Can't Open File in MZFileSystem (DropGunGameList.xml)\n");
				xmlIniData.Destroy();
				return false;
			}
		}
	...
	else
	{
		if (!mzf.Open(szFileName))
		{
			mlog("DropGunGame Can't Open File (DropGunGameList.xml)\n");
			xmlIniData.Destroy();
			return false;
		}
	}
	...
	{
		mlog("DropGunGame Can't Load (DropGunGameList.xml) From Memory\n");
		xmlIniData.Destroy();
		return false;
	}
