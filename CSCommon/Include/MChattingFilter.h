...
public:
	...
	void SetFilteringLevel(int n) { nFilteringLevel = n; }

	/// 금칙단어를 구함
	const char* GetAbuseWord()			{ return strAbuseWord.c_str();		}
	void SetAbuseWord(const char* szVal) { strAbuseWord = szVal; }
};
...
public:
	...
	string& GetLastFilteredStr();


	/// 인스턴스를 구한다
	static MChattingFilter* GetInstance();

	//If Find successfully compares strAbuseWord to word stored in xml, return pointer to that
	MAbuseWord* Find(string strAbuseWord)
	{
		//strAbuseWord.erase(remove_if(strAbuseWord.begin(), strAbuseWord.end(), ::isspace), strAbuseWord.end());
		for (auto itor = m_AbuseWordList.begin(); itor != m_AbuseWordList.end(); ++itor)
		{
			MAbuseWord* pAbuseWord = (*itor);
			if (StrStrI(strAbuseWord.c_str(), pAbuseWord->GetAbuseWord()))
			{
				return pAbuseWord;
			}
		}
	...
	list<MAbuseWord*> GetAbuseWordList()
	{
		return m_AbuseWordList;
	}
