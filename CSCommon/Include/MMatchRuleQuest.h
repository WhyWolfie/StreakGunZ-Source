...
protected:
	...
	virtual void RouteGameInfoToSingle(MUID& uidPlayer);						///< 클라이언트에 게임 정보 보내준다.
	...
	virtual void RouteMovetoPortal(const MUID& uidPlayer);
	/// 해당 플레이어가 포탈로 이동이 완료되었다고 메세지 보낸다.
	/// @param uidPlayer 이동한 플레이어 UID
	virtual void RouteReadyToNewSector(const MUID& uidPlayer);
...
public:
	...
	void SetScaling(int scaling) { m_StageGameInfo.bScaled = true; m_StageGameInfo.nScaling = 100 + scaling; }
	bool GetScaled() { return m_StageGameInfo.bScaled; }
	void ClearScaling() { m_StageGameInfo.bScaled = false; m_StageGameInfo.nScaling = 100; }
