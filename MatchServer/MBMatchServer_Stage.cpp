...
int MBMatchServer::ValidateStageJoin(const MUID& uidPlayer, const MUID& uidStage)
{
	...
	if (!IsAdminGrade(pObj) && pObj->GetAccountInfo()->m_nUGrade != MMUG_EVENTTEAM)
	{
		...
		if (pStage->GetStageSetting()->GetMaxPlayers() <= pStage->GetCountableObjCount())
		{
			...
			if (pObj->GetAccountInfo()->m_nPGrade != MMPG_PREMIUM_IP || pStage->GetRule() && pStage->GetRule()->GetGameType() == MMATCH_GAMETYPE_QUEST || pStage->GetRule() && pStage->GetRule()->GetGameType() == MMATCH_GAMETYPE_SURVIVAL || pStage->GetRule() && pStage->GetRule()->GetGameType() == MMATCH_GAMETYPE_QUEST_CHALLENGE)
				return MERR_CANNOT_JOIN_STAGE_BY_MAXPLAYERS;
		}
