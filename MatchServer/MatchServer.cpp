...
//	MNewMemories::Init();
	...
	if(m_ZFS.Create(".")==false){
		AfxMessageBox("Error de inicializa豫o de arquivos zip maiet");
		return FALSE;
	}
...
#ifdef _FETCH_112
	// sistema de mensagem em HTML no  jogo mudar aqui a  url.
...
void CMatchServerApp::OnViewServerStatus()
{
	...
	if (pServer) pServer->Log(MCommandCommunicator::LOG_PROG, "Status do Servidor ->");
