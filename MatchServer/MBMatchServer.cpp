...
bool MBMatchServer::LoadFileMD5HashValue()
{
	...
	// Custom: System MD5
	sprintf(filePath, "%s\\MD5HashValue_System.txt", curDirectory);

	ReadFp = fopen(filePath, "rb");
	if (NULL == ReadFp)
	{
		mlog("Load MD5 (System) fail.\n");
		return false;
	}
	...
	{
		fscanf(ReadFp, "%x", &readNum);
		m_szSystemMD5Value[i] = (unsigned char)readNum;
	}

	fclose(ReadFp);


	mlog("FILE PATH : %s\nMD5 (System) Hash Value => ", filePath);
	for (int i = 0; i < 16; i++)
	{
		mlog("%02x ", m_szSystemMD5Value[i]);
	}
...
/*
	// Custom: Gunz64 MD5
	sprintf(filePath, "%s\\MD5HashValue_Gunz64.txt", curDirectory);

	ReadFp = fopen(filePath, "rb");
	if( NULL == ReadFp )
	{
		mlog( "Load MD5 (Gunz64) fail.\n" );
		return false;
	}
	...
	{
		fscanf(ReadFp, "%x", &readNum);
		m_szGunz64MD5Value[i] = (unsigned char)readNum;
	}
	
	fclose(ReadFp);

	mlog("FILE PATH : %s\nMD5 (Gunz64) Hash Value => ", filePath);
	for( int i=0; i<16; i++)
	{
		mlog("%02x ", m_szGunz64MD5Value[i]);
	}

	mlog("\n");

	// Custom: Update MD5
	sprintf(filePath, "%s\\MD5HashValue_Update.txt", curDirectory);

	ReadFp = fopen(filePath, "rb");
	if (NULL == ReadFp)
	{
		mlog("Load MD5 (Update) fail.\n");
		return false;
	}
	...
	{
		fscanf(ReadFp, "%x", &readNum);
		m_szUpdateMD5Value[i] = (unsigned char)readNum;
	}

	fclose(ReadFp);


	mlog("FILE PATH : %s\nMD5 (Update) Hash Value => ", filePath);
	for (int i = 0; i<16; i++)
	{
		mlog("%02x ", m_szUpdateMD5Value[i]);
	}

	mlog("\n");

	// Custom: Secret String
	sprintf(filePath, "%s\\MD5HashValue_Secret.txt", curDirectory);

	ReadFp = fopen(filePath, "r");
	if( NULL == ReadFp )
	{
		mlog( "Load MD5 (Secret) fail.\n" );
		return false;
	}

	char line[16];
	memset(line, 0, sizeof(line));
	fgets(line, 16, ReadFp); // we only want the first line
	line[15] = '\0';

	memcpy(m_szSecretStringValue, line, 16);

	//readNum = 0;
	//for (int i = 0; i < 16; i++)
	//{
	//	fscanf(ReadFp, "%x", &readNum);
	//	m_szSecretStringValue[i] = (unsigned char)readNum;
	//}
	...
	mlog("FILE PATH : %s\Secret Hash Value => ", filePath);
	for( int i=0; i<16; i++)
	{
		mlog("%02x ", m_szSecretStringValue[i]);
	}

	mlog("\n");

	// Custom: Antihack
	sprintf(filePath, "%s\\MD5HashValue_AH.txt", curDirectory);

	ReadFp = fopen(filePath, "rb");
	if( NULL == ReadFp )
	{
		mlog( "Load MD5 (AH) fail.\n" );
		return false;
	}
	...
	{
		fscanf(ReadFp, "%x", &readNum);
		m_szAHMD5Value[i] = (unsigned char)readNum;
	}
	...
	mlog("FILE PATH : %s\AH Value => ", filePath);
	for( int i=0; i<16; i++)
	{
		mlog("%02x ", m_szAHMD5Value[i]);
	}
