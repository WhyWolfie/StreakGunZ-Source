...
DWORD GetFileCheckSum(char* pszFileName)
{
	...
	DWORD dwCheckSum = 0, dwBytesToRead;

	BY_HANDLE_FILE_INFORMATION fileInfo;
	GetFileInformationByHandle(hFile, &fileInfo);

	szBuffer = new char[fileInfo.nFileSizeLow];

	if (ReadFile(hFile, szBuffer, fileInfo.nFileSizeLow, &dwBytesToRead, 0) == FALSE)
		return 0;

	dwCheckSum = MCRC32::BuildCRC32((unsigned char*)szBuffer, fileInfo.nFileSizeLow);
...
DWORD GetCRC( const char* szFileName)
{
	DWORD dwCRC = 0;
	fpos_t nSize;
	...
	dwCRC = MCRC32::BuildCRC32((unsigned char*)pBuffer, nSize);
