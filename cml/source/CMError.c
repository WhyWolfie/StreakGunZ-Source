...
static char g_pLastCustom[256];	/*	FileName's Last Custom that Error Occured	*/
...
에러 지정
	...
	pLastCustom	에러가 일어난 파일의 최근 수정일(__TIMESTAMP__)
	pErrStr				에러 스트링(에러코드 + _KSTR or _ESTR)
*/
void _SetError(int nErrCode,const char *pErrSubStr,const char *pFileName,int nLineNum,const char *pLastCustom,const char *pErrStr)
{
#ifdef _WIN32
	LPVOID lpMsgBuf;
#endif
	...
	g_nLineNum=nLineNum;
	strcpy(g_pLastCustom,pLastCustom);
...
*/
char *GetLastCustom(void)
{
	return g_pLastCustom;
}
...
void ErrMsgBox(HWND hWnd)
{
	...
	sprintf(strTemp,"%s\nLast Modify Date		: %s",strTemp,g_pLastCustom);
#endif
