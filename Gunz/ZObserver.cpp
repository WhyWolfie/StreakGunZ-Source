...
void ZObserver::DrawPlayerDuelHPAPBar(MDrawContext* pDC)
{
	...
	//if (RGetIsLongScreen())
	//	pDC->FillRectangleW( (218.0f+150.0f)*fRx-nWidth-(22/1280*RGetScreenHeight()), 37.0f*fRy, nWidth, 5.0f*fRy);
	//else
	...
	//if (RGetIsLongScreen())
	//	pDC->FillRectangleW( 432.0f*fRx+(22/1280*RGetScreenHeight()), 37.0f*fRy, nWidth, 5.0f*fRy);
	//else
	...
	if ( pBitmap)
	{
		...
		//{
		//	pDC->Draw( ( 800*167.0f*fRx+80*RGetScreenWidth() )/960.f+1.f, 0, 466.0f*fRy+57, 49.0f*fRy); // debug
		//	//pDC->Draw( ( 800*167.0f*fRx+90*RGetScreenWidth() )/960.f, 0, 466.0f*fRy, 49.0f*fRy); // 570.0f*fRy (ASM REVERSE)
		//}
		else
	...
	if ( bExistNextChallenger)
	{
		...
		// Custom: Long screen update
		//if (RGetIsLongScreen())
		//	TextRelative( pDC, 0.89f, 0.01f, charName[ 2], false);
		//else
...
//void ZObserver::SetKillCam(bool bIsKillCam)
//{
//	m_bIsKillCam = bIsKillCam;
//}
