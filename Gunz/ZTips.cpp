...
bool ZTips::Initialize(MZFileSystem* pfs, const MLanguage LangID )
{
	...
	// If the selected languages tips fails to load, load, default to system
	if (!mzf.Open(szTipsPath, pfs))
	{
		mlog("Error: Failed to load tips from selected language. Defaulting to English.\n");
		if (!mzf.Open("system/tips.xml", pfs))
		{
			mlog("Error: Failed to load tips from the default path, closing Gunz.\n");
			return false;
		}
		else
			mlog("Load XML from memory : %s(0x%04X)\n", "system/tips.xml", LangID);
	}
	else
		mlog("Load XML from memory : %s(0x%04x)\n", szTipsPath, LangID);
