...
//				pCommand->GetParameter(szProposerCharName,	1, MPT_STR);
		...
		case MC_MATCH_PLAYERWARS_COUNTER:
		{
			if (ZGetGameInterface()->GetState() == GUNZ_LOBBY)
			{
				...
				for (int i = 0; i <= 7; i++)
				{
					sprintf(szWigetName, "Lobby_StreakLadderCount%d", i);
		...
		case MC_MATCH_NOTIFY_LADDER_REJOIN:
			{
				//ZChatOutput( ZMsg(MSG_LADDER_REJOIN_TEXT), ZChat::CMT_SYSTEM );
				//ZApplication::GetGameInterface()->ShowConfirmMessage(ZMsg(MSG_LADDER_REJOIN_DIALOG), ZGetLadderRejoinConfirmListener());
			}
			break;
		...
		{
			//MUID uidObj;
			int nMedals; // no es nLC?

			//pCommand->GetParameter(&uidObj, 0, MPT_UID);
			pCommand->GetParameter(&nMedals, 0, MPT_INT);

			char szBuffer[256];
			sprintf(szBuffer, "(1) Actual Medals : %d\n", ZGetMyInfo()->GetLC());
			mlog(szBuffer);

			ZGetMyInfo()->SetLC(ZGetMyInfo()->GetLC() + nMedals);
