...
BEGIN_IMPLEMENT_LISTENER(ZGetBattleExitButtonListener, MBTN_CLK_MSG)
	// 클랜전이 진행중이면 종료하지 않는다
	if (!ZGetGameClient()->IsCWChannel() && !ZGetGameClient()->IsPWChannel())
	{
		if(pWidget->GetParent()!=NULL) pWidget->GetParent()->Show(false);
...
class ZLadderRejoinConfirmListener : public MListener{
public:
	virtual bool OnCommand(MWidget* pWidget, const char* szMessage){
		if(MWidget::IsMsg(szMessage, MMSGBOX_YES)==true)
		{
...
} g_LadderRejoinConfirmListener;

MListener* ZGetLadderRejoinConfirmListener()
{
	return &g_LadderRejoinConfirmListener;
}
