...
bool ZGameInterface::OnGameCreate(void)
{
	...
	case (8):
		...
		break;
	case (9):
		strcpy(szFileName, "Interface/Default/LOADING/loading_grade_fifteen.jpg");
		break;
	case (10):
...
//		pServerList->AddServer( "Billcap", "192.168.0.132", 6000, 2, 0, 1000, true);
	...
	//		pServerList->AddServer( "Debug Server", "127.0.0.1", 6000, 4, 0, 1000, true);			// Debug server
		...
		if (pServerList->GetServerList().size() == 0)
		{
			...
			pServerList->AddServer("East Coast (Backup...)", const_cast<char*>(strIP.c_str()), 6000, 3, 0, 1, true, const_cast<char*>(strIP.c_str()), 0, 0, 5100);
		}
#endif
#else
...
void ZGameInterface::OnLobbyCreate(void)
{
	...
	/*if (ZGetMyInfo()->IsAdminGrade())
		ShowWidget("ChannelList_PlayerWars", false);
	else
		ShowWidget("ChannelList_PlayerWars", true);*/
...
void ZGameInterface::InitPlayerWarsUI(bool bPlayerWars)
{
	BackGroundButtom();
	...
	if (pWidget) pWidget->Show(!bPlayerWars);
	pWidget = m_IDLResource.FindWidget("StageAfterBtn");
	if (pWidget) pWidget->Show(!bPlayerWars);
	bool bCheck = !bPlayerWars;
	for (int i = 1; i <= 6; i++)
	{
		char szBuffer[64];
	...
	m_CombatMenu.EnableItem(ZCombatMenu::ZCMI_BATTLE_EXIT, !bPlayerWars);
...
void ZGameInterface::InitLobbyUIByChannelType()
{
	...
	// ¼ø¼­°¡ Áß¿äÇÏ´Ù.. trueÀÎÂÊÀ» ³ªÁß¿¡ È£ÃâÇØÁà¾ß ÇÑ´Ù (ÃßÇÏÁö¸¸ ÀÌÁ¦¿Í¼­ ÇÔ¼EÇÑ±ºµ¥·Î ÇÕÃÄÁÖ±âµµ Á» µÎ·Á¿E..)
	if (bClanBattleUI == true)
		SelectBackground(1);
	else if (bDuelTournamentUI == true)
		SelectBackground(2);
...
*/
	...
	if ((nType == ZCWT_PREV) || (nType == ZCWT_NEXT))
	{
		...
		// °¡ÁE¾ÆÀÌÅÛ ¸ñ·ÏÀ» ¸¸µé°E ÇöÀEµé°úÜÖ´Â ³ÑÀ» Ã£´Â´Ù
		for(int i = MMCIP_MELEE; i < MMCIP_CUSTOM2 + 1; i++)
		{
			if (ZGetConfiguration()->GetEtc()->bWeaponRotation && (i == MMCIP_CUSTOM1 || i == MMCIP_CUSTOM2))
				continue;
		...
		if (nType == ZCWT_PREV)
		{
			...
			while ( nNewPos != nPos)
			{
				...
				if ( !ZGetConfiguration()->GetEtc()->bWeaponRotation && ((nPart == MMCIP_CUSTOM1) || (nPart == MMCIP_CUSTOM2)) ) {
...
bool ZGameInterface::Update(float fElapsed)
{
	...
	// Custom: kick when people are AFK in the stage for more than 15 minutes.
...
void ZGameInterface::UpdateCursorEnable()
{
	...
	if( GetState()!=GUNZ_GAME ||
		...
		GetRGMain().IsCursorEnabled() // Custom: Enbale toggle Cursor Chat to Move
		)
		SetCursorEnable(true);
	else
	{
...
void ZGameInterface::InitDuelTournamentLobbyUI(bool bEnableDuelTournamentUI)
{
	...
	if(pWidget) pWidget->Show(false);
#else	// Å×½ºÆ®¿E2,4ÀÎ µà¾óÅä³Ê¸ÕÆ® ÂE¡ ¹öÆ°
	pWidget= m_IDLResource.FindWidget( "DuelTournamentGame_2Test" );
	if(pWidget) pWidget->Show(bEnableDuelTournamentUI);
	pWidget= m_IDLResource.FindWidget( "DuelTournamentGame_4Test" );
	if(pWidget) pWidget->Show(bEnableDuelTournamentUI);
...
#endif // _CW_VOTE
{
	...
	if (isvote == true)
	{
		...
		if (pWidget)
		{
			...
			if (ZGetGameClient()->GetChannelType() == MCHANNEL_TYPE_CLAN || ZGetGameClient()->GetChannelType() == MCHANNEL_TYPE_PLAYERWARS)
...
//	SAFE_ENABLE("LobbyPlayerListTabClanCreateButton");
	...
	SAFE_ENABLE("ArrangedPlayerWars", bFinding); // Custom: Player Wars invisible button
...
void ZGameInterface::OnDuelTournamentGameUI(bool bWaiting)
{
	...
	SAFE_ENABLE("ArrangedPlayerWars", bWaiting); // Custom: Player Wars invisible button
#ifdef _LADDERUPDATE 
...
#define FLUSH				if(buffer[0]) { Text(r.x+nLastX+xRight, y, buffer); FLUSHPOS(r.x+nLastX); nLastX=nX; buffer[0]=0;pcurbuf=buffer; }
				...
				for (int i = 0; i < nCharCount; i++) {
					...
					/*else if (c == ':' && ('sg' == cc || 'sg' == cc))
					{

						if (strlen(szText) == strlen(ZGetMyInfo()->GetCharName()) + 5) {
							FLUSH;
							SetBitmap(MBitmapManager::Get("streak.png"));
							Draw((r.x + nX), y, (float)RGetScreenWidth() / (float)800 * 11, (float)RGetScreenWidth() / (float)800 * 11);
							c = ' ';
							cc = ' ';
							szCurrent[i] = ' ';
