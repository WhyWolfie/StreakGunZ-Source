	...
	case MC_MATCH_RESPONSE_CHARACTER_ITEMLIST:
		{
			...
			/*pCommand->GetParameter(&nMedal, paramindex++, MPT_INT);
			ZGetMyInfo()->SetMedal(nMedal);*/
	...
	case MC_MATCH_RESPONSE_BUY_ITEM: //Jork
		{
			...
			else if ((nResult == MERR_TOO_EXPENSIVE_BOUNTY) || (nResult == MERR_TOO_EXPENSIVE_CASH) || (nResult == MERR_TOO_EXPENSIVE_MEDAL) || (nResult == MERR_TOO_MANY_ITEM))
			{
				ZApplication::GetGameInterface()->ShowErrorMessage( nResult );
			}
			else
