...
void ZInput::Update()
{
	...
	// Custom: Sniper scope support for mouse sensitivity w/o messing up config
	float fMouseSens = Z_MOUSE_SENSITIVITY;

	if( m_bSnifer )
	{
		int nSens = (int)((float)0.5f * (float)ZGetConfiguration()->GetMouseSensitivityInInt());
		int validated = ZGetConfiguration()->ValidateMouseSensitivityInInt(nSens);

		fMouseSens = validated / (float)MOUSE_SENSITIVITY_MAX;
	}

	float fRotateStep = 0.005f * fMouseSens;
