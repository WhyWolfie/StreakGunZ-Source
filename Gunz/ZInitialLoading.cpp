								 ...
								 float sx_, float sy_, float sw_, float sh_, bool bTipsVisible)
{
	...
	// √ªº“≥ÅE¿⁄¿≤ ±‘¡¶ ¿˚øÅE»(æ≤π˙æ≤π˙æ≤πÅE..)...
#ifdef LOCALE_KOREA
	if ( (rand() % 2) == 0)
		sprintf( m_szNotice, "¿ÂΩ√∞£¿« ∞‘¿” ¿ÃøÅE∫ ∞«∞≠¿ª «ÿƒ• ºÅE¿÷Ω¿¥œ¥Ÿ.");
	else
		sprintf( m_szNotice, "∞¯∫ŒøÕ ¿œø° »ﬁΩƒ¿Ã « ø‰«œµ˙‹Ã, ∞‘¿” ¿ÃøÅE°µµ ¿˚¿˝«— »ﬁΩƒ¿Ã « øÅE«’¥œ¥Ÿ.");
...
#ifdef _DEBUG // »§Ω√ µ«µπæ∆∞°¥¬∞ÊøÅEŒ¡ÅE√º≈©
...
void	ZInitialLoading::Draw( LOADING_SCENE_MODE mode_ /* = MODE_DEFAULT */, int destIndex_ /* = -1 */, bool bFlip_, bool fromThread_ )
{
	...
	if( !m_bGradeFadeOutStart && timeGetTime() - m_Grade_Time > GRADE_TIMER-2 )	// 3√  µøæ»∏∏ ∫∏ø©¡‹
	{	// 3√ »ƒø° 15ºº µ˚Õﬁ ¿ÃπÃ¡ÅE∆‰¿ÃµÅEæ∆øÙ√≥∏Æ Ω√¿€«ÿ ¡‹
		numDrawing				= FADEOUT_NUM_DRAWING;
...
{
    if( !mbUseEnable ) return;	// »∞º∫»≠ µ«æÅE¿÷¡ÅEæ ¿∏∏ÅEskip
...
//	return;			// «ˆ¿ÅE¿˚øÅE»µ 
	...
	if ((g_pDC != 0) && (pFont != 0))
	{
		...
		else
		{
			...
			// ∑ŒµÅE»≠∏Èø° ∏¬√Áº≠ ≈ÿΩ∫∆Æ∏¶ º≥¡§«—¥Ÿ.(«œµÂƒ⁄µÅE.. -_-;)
			int nPosx, nPosy;
