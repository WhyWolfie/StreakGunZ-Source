...
void ZMatch::SoloSpawn()
{
	...
	if (ZGetGame()->m_pMyCharacter->IsDie())
	{
		...
		if (ZGetGame()->GetMatch()->IsTeamPlay() && ZGetGameClient()->IsCWChannel() && ZGetGameClient()->IsPWChannel() && ZGetGame()->GetMatch()->GetMatchType() != MMATCH_GAMETYPE_SPY)
		{
...
void ZMatch::OnForcedEntry(ZCharacter* pCharacter)
{
	...
	if (pCharacter == ZGetGame()->m_pMyCharacter)
	{
		...
		if (pObjCache && pObjCache->CheckFlag(MTD_PlayerFlags_AdminHide)) {
			...
			// 팀플레이면 스팩테이터
			if (IsWaitForRoundEnd() || IsQuestDrived() || GetMatchType() == MMATCH_GAMETYPE_QUEST_CHALLENGE)
			{
				pCharacter->SetVisible(false);
				pCharacter->ForceDie();
	...
	else
	{
		...
		// 단 FREE상태일 때에는 보여준다
		if (IsWaitForRoundEnd() && (GetRoundState() != MMATCH_ROUNDSTATE_FREE) || IsQuestDrived() || GetMatchType() == MMATCH_GAMETYPE_QUEST_CHALLENGE)
		{
			if (pCharacter != NULL)
			{
				pCharacter->SetVisible(false);
